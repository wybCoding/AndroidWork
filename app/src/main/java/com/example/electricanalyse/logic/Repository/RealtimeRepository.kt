...
interface RealtimeRepositoryInter {
    suspend fun getApplianceList(): MutableList<Appliance>
}

object RealtimeRepository : RealtimeRepositoryInter{

    override suspend fun getApplianceList(): MutableList<Appliance> {
        // 生成随机的家电列表
        val list = mutableListOf<Appliance>()
        for (i in 1..4) { // 生成10个随机的家电对象
            val name = "produce ${Random.nextInt(1, 101)}" // 随机生成家电名称
            val date = "2024-${Random.nextInt(1, 13)}-${Random.nextInt(1, 32)}" // 随机生成日期
